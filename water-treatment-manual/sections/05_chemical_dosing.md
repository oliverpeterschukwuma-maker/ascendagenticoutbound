...
### How to dose — THREE injection points on this site

`ℹ️ CONFIRMED BY OPERATOR:` Unlike PACl (added once, indoors), flocculant on this site is added at **three separate points** along the treatment train, not just one:
...
`ℹ️ WHY THREE POINTS, NOT ONE:` Each point is treating water that has already lost some solids to the step before it, but is also picking up disturbance/mixing from pumping and baffles. Re-dosing flocculant at each major transfer point keeps floc forming continuously instead of relying on one dose to last the entire journey from pit to indoor tanks. Think of it like reapplying glue at each stage of an assembly line, rather than gluing everything once at the very end.
