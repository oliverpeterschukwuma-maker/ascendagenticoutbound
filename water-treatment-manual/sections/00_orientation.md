...
> Oil/water cell → frac tank (settles) → pumped to the **"C Can"** (= the sea can — **confirmed to physically exist on site**) → inside the C Can, water gets a **pH check first — lime (TEXLOME High Calcium Hydrated Lime) is only added if the pH reading is off**, before PACl coagulant + flocculant go in; if pH is already fine, lime is skipped (see Section 5.6a) → flows to the **weir tank, which IS one of the blue bins — only 1 blue bin is currently in use**, not 2 → settles in the weir tank/blue bin → pumped out to the intake pump → **ozone** injected → *(steps after this are still being confirmed)*.
>
