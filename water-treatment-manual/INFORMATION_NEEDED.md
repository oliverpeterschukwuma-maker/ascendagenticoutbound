...
- [ ] **Sand filter (SRS-SF-11) is broken** — root cause narrowed down by Brennan: the grey actuating solenoids (black tubing) likely aren't firing — board/timer logic tests fine, but no click/valve movement. Brennan doesn't know how to repair solenoids himself; at his last site this required an electrician. Action: get an electrician in to test/replace the solenoids. Until fixed, the bag filter is carrying extra load it wasn't designed for — ask if discharge water quality is being checked more closely in the meantime.
- [ ] **Ozone position + missing peroxide/chlorination — reconcile two conflicting accounts.** A former operator says ozone runs FIRST (before any chemicals) and doesn't mention hydrogen peroxide dosing or chlorination at all. Earlier photo-based notes had ozone LAST (after carbon, before chlorination) with peroxide dosed at the white tanks. Ask Brennan/Rob/Glen directly: is chlorination (Accu-Tab) still part of the live process? Is hydrogen peroxide still dosed? Where does ozone actually sit in the sequence today? This affects whether the water is being disinfected before discharge — a compliance-relevant question.
- [ ] **Discharge permit / BC EMA authorization** — the actual legal limits (pH, turbidity, iron, TPH, suspended solids). Ask supervisor or SUMAS.
- [ ] **Where treated water is discharged** — storm sewer, creek, ground? What is the receiving environment?
- [ ] **Permit exceedance reporting** — who to call, what form to use, within what timeframe
...
- [ ] **Dosing pump settings** — current stroke rate and stroke length on each pump. Operator confirmed dose adjustment at the C Can is done via ongoing test results (add more coagulant/flocculant if under-dosing) and via stroke rate/length on the pump. **Photos of the 3 C Can dosing pumps (coagulant, flocculant, lime) coming** — update Section 5.1b once received.
...
- [x] **3rd chemical at the C Can confirmed: TEXLOME High Calcium Hydrated Lime.** Target pH 8+ is good for the coagulant; below 8 is not. Lime goes in before PACl + flocculant.
- [ ] **⚠️ UNRESOLVED — conditional vs. every-time:** Operator first said lime is added only when pH is off (below 8). Most recently said **"we pump the lime every time because the coagulant lowers the pH."** Ask directly: do you check pH each time before deciding whether to add lime, or is lime now a fixed step on every batch regardless of the reading? Still also need: exact dose/ratio of lime, SDS, PPE (see Section 5.6a).
...
- [ ] **C Can → blue bin (weir tank) discharge connection — still needs tracing.** Now that the base hose is confirmed as the frac-tank inflow, how does dosed water actually get from the C Can into the adjacent blue bin?
...
- **Discharge destination clarified** (former operator): treated water → holding tanks → trucked/transferred to a lagoon for disposal (not necessarily a permitted point-source discharge — needs reconciling with the discharge permit question above).
...
- [x] **TEXLOME High Calcium Hydrated Lime — confirmed as the "3rd chemical" at the C Can, dosed first.** Still need exact dose, SDS, PPE.
